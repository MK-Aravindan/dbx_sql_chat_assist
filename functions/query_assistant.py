...
system_prompt = (
    """
        ...
        While returning the SQL query, return the table names with `<catalog>.<schema>.<table>` (for example: `main`.`sales`.`orders`); always use this format for all table references.
...
# ==== Agent Factory ====
def catalog_metadata_agent(system_prompt: str, model: str="openai:gpt-4.1-mini") -> Agent:
    return Agent(
        model=model,
